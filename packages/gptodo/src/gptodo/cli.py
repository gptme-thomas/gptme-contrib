...
def _task_project(task) -> str:
    """Project name from frontmatter (optional convention; empty if unset)."""
...
def _get_browse_lines(tasks_dir, sort_key="date", filter_state=None, project=None, show_all=False):
    ...
    if project:
        tasks = [t for t in tasks if _task_project(t) == project]
    ...
    col_proj = max(max(len(_task_project(t)) for t in tasks), len("PROJECT"))
    ...
    for task in tasks:
        ...
        project_str = _task_project(task)
...
def _browse_pager(tasks, repo_root):
    ...
    for i, task in enumerate(tasks):
        ...
        if _task_project(task):
            parts.append(f"  Project:  {_task_project(task)}")
        if task.tags:
